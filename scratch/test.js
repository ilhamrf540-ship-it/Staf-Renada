    ...
    // Kompres gambar ke thumbnail kecil (max 150px, JPEG quality 0.6) â†’ base64 ~5-10KB
    ...
    function saveGoogleSheetsConfig() {
      ...
      if (!url) {
        statusMsg.textContent = "âŒ Harap masukkan URL Web App yang valid.";
        statusMsg.style.color = "var(--error)";
        return;
      }
      
      if (!url.startsWith("https://script.google.com/")) {
        statusMsg.textContent = "âŒ URL harus berawalan dari Google Script (https://script.google.com/)";
        statusMsg.style.color = "var(--error)";
        return;
      }
      
      localStorage.setItem('google_sheets_api_url', url);
      statusMsg.textContent = "â³ Mencoba menghubungkan ke Google Sheets...";
      ...
      fetch(url + "?action=getUsers")
        .then(res => {
          if (res.ok) {
            statusMsg.textContent = "âœ… Sukses terhubung ke Google Sheets! Sinkronisasi aktif.";
        ...
        .catch(err => {
          statusMsg.textContent = "âš ï¸ Gagal terhubung. Pastikan Web App Anda di-deploy dengan akses 'Anyone'.";
          statusMsg.style.color = "var(--error)";
        });
    }
    ...
    function disconnectGoogleSheets() {
      ...
      const statusMsg = document.getElementById('sheetsStatusMessage');
      statusMsg.textContent = "ðŸ”Œ Sinkronisasi dinonaktifkan. Menggunakan penyimpanan lokal browser (LocalStorage).";
      statusMsg.style.color = "var(--text-muted)";
      setTimeout(() => {
        location.reload();
      }, 1500);
    ...
    function checkGoogleSheetsStatus() {
      ...
      if (url && inputEl && disBtn && statusMsg) {
        ...
        statusMsg.textContent = "âœ… Terhubung ke Google Sheets (Sinkronisasi Aktif)";
        statusMsg.style.color = "var(--success)";
      }
    }
    ...
    async function showPanelForRole(role) {
      ...
      if (role === 'siswa') {
        ...
        await switchAdminTab('personel');
      }
    }

    async function switchAdminTab(tabName) {
      const tabs = ['personel', 'soal', 'absensi', 'hasil', 'sync', 'tampilan'];
      tabs.forEach(t => {
    ...
    async function handleProfilePhotoChange(input) {
      ...
      try {
        // Kompres ke thumbnail 150x150 JPEG quality 0.6 (âˆ¼5-10KB)
    ...
    async function saveProfile(event) {
      ...
      if (!nama) {
        msgEl.textContent = 'âŒ Nama lengkap tidak boleh kosong.';
      ...
      if (passBaru && passBaru.length < 4) {
        msgEl.textContent = 'G¥î Password baru minimal 4 karakter.';
        msgEl.style.color = 'var(--error)';
        msgEl.style.background = '#fef2f2';
        msgEl.style.display = 'block';
        return;
      ...
      if (passBaru && passBaru !== passKonfirm) {
        msgEl.textContent = 'G¥î Konfirmasi password tidak cocok.';
        msgEl.style.color = 'var(--error)';
        msgEl.style.background = '#fef2f2';
        msgEl.style.display = 'block';
        return;
      ...
      msgEl.textContent = 'G£à Profil berhasil diperbarui!';
      ...
      questions.forEach((q, index) => {
        ...
        if (q.fileData) {
          fileLinkHtml = `<div style="margin-top: 0.5rem; margin-bottom: 1rem; font-size: 0.85rem; background: var(--primary-light); padding: 0.5rem; border-radius: var(--radius-sm); border: 1px dashed var(--primary);">
            =ƒôä <a href="${q.fileData}" download="${q.fileName}" style="color:var(--primary); font-weight:700; text-decoration:underline;">Unduh Dokumen Soal (${q.fileName})</a>
    ...
    async function submitExam(event) {
      ...
      try {
        ...
        alert('G£à Berhasil! Jawaban ujian dan dokumen Anda telah sukses terkirim ke Operator.');
      ...
      questions.forEach((q, index) => {
        ...
        if (q.fileData) {
          fileLinkHtml = `<div style="font-size: 0.8rem; margin-top: 0.35rem;">
            =ƒôä Dokumen: <a href="${q.fileData}" download="${q.fileName}" style="color:var(--accent); font-weight:700; text-decoration:underline;">${q.fileName}</a>
      ...
      Object.keys(sub.answers).forEach((qId, index) => {
        ...
        if (sub.answerFiles && sub.answerFiles[qId]) {
          ...
          fileLink = `<div style="font-size:0.8rem; margin-top:0.5rem; background: var(--primary-light); padding: 0.4rem; border-radius: 4px; display: block; width: fit-content;">
            =ƒôä File Jawaban: <a href="${fileObj.fileData}" download="${fileObj.fileName}" style="color:var(--primary); font-weight:700; text-decoration:underline;">${fileObj.fileName}</a>
        ...
        answersHtml += `
          <div style="margin-bottom: 0.85rem; padding: 0.5rem; background: ${isCorrect ? 'var(--accent-light)' : '#fef2f2'}; border-radius: 4px;">
            ...
            <div style="font-size: 0.85rem; margin-top: 0.25rem;">
              ...
              ${fileLink}
              <span style="font-size:0.75rem; display:block; margin-top:0.25rem;">Pencocokan Kata Kunci: ${isCorrect ? '=ƒƒó Cocok' : '=ƒö¦ Tidak Cocok'} ${keywords.length > 0 ? `(Target: ${keywords.join('/')})` : ''}</span>
            </div>
          </div>
        `;
      });
    ...
    async function renderAdminPersonnel() {
      ...
      if (!tbody) return;
      tbody.innerHTML = `<tr><td colspan="7" style="text-align:center; color:var(--text-muted); padding:1.5rem;">GÅ¦ Memuat data personil...</td></tr>`;
